...
describe('suite sync model', () => {
  ...
  it('keeps the newest payload independently for every app', () => {
    const merged = mergeSuiteStates(
      { apps: { dashboard: { value: { title: '旧' }, updatedAt: 1, updatedBy: 'a' }, checklist: { value: { appName: '本地', templates: [], projects: [] }, updatedAt: 4, updatedBy: 'a' } } },
      { apps: { dashboard: { value: { title: '新' }, updatedAt: 2, updatedBy: 'b' }, checklist: { value: { appName: '云端', templates: [], projects: [] }, updatedAt: 3, updatedBy: 'b' } } },
    )
